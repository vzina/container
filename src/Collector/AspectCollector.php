...
class AspectCollector extends MetadataCollector
{
    ...
    public static function clear(?string $key = null): void
    {
        if ($key) {
            unset(
                static::$container['classes'][$key],
                static::$container['annotations'][$key],
                static::$aspectRules[$key]
            );
        } else {
            static::$container = [];
            static::$aspectRules = [];
    ...
    public static function load(string $className): array
    {
        ...
        $keys = ['classes', 'annotations', 'priority'];
        $ret = [];
        foreach ($properties as $property) {
            $name = $property->getName();
            if (in_array($name, $keys)) {
                $ret[$name] = ReflectionManager::getPropertyDefaultValue($property);
            }
        }
